...
find_latest_zip <- function(base_url, start_date, max_days = 365) {
  ...
  for (step in step_sizes) {
    ...
    if (!found_at_least_one) {
      # Nenhum encontrado nesse bloco, desiste
      next
    }
  }
...
download.file(zip_url, basename(zip_url), mode = "wb")

# Listar arquivos dentro do ZIP
zip_contents <- archive::archive(zip_file)$path

# Encontrar o CSV desejado
csv_file <- zip_contents[grepl("msp_alvaras_sisacoe_aprovadigital.*\\.csv$", zip_contents)]

if (length(csv_file) == 0) {
  stop("Nenhum arquivo CSV correspondente encontrado no ZIP.")
}
...
# Lendo o arquivo
alvaras_raw <- read.csv2(csv_file,fileEncoding = "latin1", stringsAsFactors = FALSE)
...
alvaras_raw <- alvaras_raw %>%
  mutate(ano = ano_emissao, mes = ymd(paste(year(data_emissao), month(data_emissao), 1, sep = "-")))

# Renomeia colunas
alvaras_raw <- alvaras_raw %>%
  rename(alvara = n_documento, descricao = assunto, unidade_pmsp = unid_aprov, 
         sql_incra = sql, zona_de_uso_registro = zona_uso, categoria_de_uso = uso_subcat,
         area_do_terreno = area_terreno, area_da_construcao = ac_total, n_blocos = blocos,
         n_pavimentos_por_bloco = pavimentos, n_unidades = unid_resid, data_aprovacao = data_emissao, ano_aprovacao = ano_emissao)

# Cria coluna de unidades por bloco e de número total de pavimentos
alvaras_raw <- alvaras_raw %>%
  mutate(n_unidades_por_bloco = n_unidades / n_blocos,
...
alvaras_key <- alvaras_trimed %>%
  select(-c(# datas
    ...
    "n_blocos", "n_pavimentos_por_bloco", "n_unidades_por_bloco", "n_pavimentos", 
    "n_unidades", "unid_his", "unid_hmp", "unid_r2h_r2v", "coord_x", "coord_y",
    # categoricas
...
atts_numericos <- alvaras_trimed %>%
  select(id, starts_with("n_"), starts_with("area_"), starts_with("unid_"), starts_with("coord_")) %>%
...
att_descricao <- alvaras_trimed %>%
  transmute(
    ...
    ind_loteamento = if_else(
      str_detect(descricao,"DESMEMBRAMENTO|LOTEAMENTO|DESDOBRO|REMEMBRAMENTO|TERMO DE VERIF|Desmembramento|Loteamento"),
      TRUE, FALSE),
    ind_aprovacao = if_else(str_detect(descricao, "APROVACAO|Aprovação"), TRUE, FALSE),
    ind_execucao = if_else(str_detect(descricao, "EXECUCAO|Execução"), TRUE, FALSE),
    ind_conclusao = if_else(str_detect(descricao, "CONCLUSAO|Conclusão"),
                            TRUE, FALSE),
    ind_correcao = if_else(
      str_detect(descricao,"APOSTILAMENTO|PROJETO MODIFICATIVO|Apostilamento|Projeto Modificativo"),
      TRUE, FALSE),
    ind_edificacao_nova = if_else(str_detect(
      descricao, "EDIFICACAO NOVA|EDIFICACAONOVA|EDI-FICACAO NOVA|Edificação Nova"), TRUE, FALSE),
...
att_sql_incra <- alvaras_trimed %>%
  transmute(
    id, sql_incra, #nchar_sql_incra = nchar(sql_incra), 
    sql_incra_11 = case_when(nchar(sql_incra) == 0 ~ NA_character_,
                             nchar(sql_incra) > 11 ~ substr(sql_incra, 1, 11),
                             TRUE ~ sql_incra),
    ind_sql_incra_null = if_else(is.na(sql_incra_11), 
                                 TRUE, FALSE), 
    ind_incra = if_else(ind_sql_incra_null == FALSE,
                        TRUE, FALSE),
  )
...
att_endereco <- alvaras_trimed %>%
  transmute(
    id, 
    data_aprovacao = case_when(nchar(data_aprovacao) == 0  ~ NA_Date_,  # Se não há caracteres, atribui NA
                               nchar(data_aprovacao) == 10 ~ as_date(str_replace_all(data_aprovacao, "/", "-")),  # Se tem 10 caracteres, converte para data
                               TRUE ~ NA_Date_ ),
    endereco_raw = endereco,
    endereco_unico = case_when(nchar(sql_incra) > 11  ~ sub(";.*", "", endereco),
                               TRUE ~ endereco),
    ...
    numero = case_when(
      ...
      str_detect(endereco_ajust_1, "[0-9]\\'") ~ str_extract(endereco_unico, "\\'.*") %>%
        str_extract(pattern = "[-+]?[0-9]*\\.?[0-9]+") %>%
        as.numeric() %>% as.character(),
      
      # Casos onde o número está entre duas vírgulas em um intervalo
      str_detect(endereco_ajust_1, ",\\s*\\d{1,5}(\\.\\d{3})?,") ~ str_extract(endereco_ajust_1, ",\\s*\\d{1,5}(\\.\\d{3})?") %>%
...
alvaras_trusted_not_unique <- list(alvaras_key, atts_datas, atts_numericos, att_categoricos) %>%
  ...
  select(
    ...
    area_do_terreno, area_da_construcao, n_blocos, n_pavimentos_por_bloco, 
    n_unidades_por_bloco, n_pavimentos, n_unidades, n_unidades, unid_his, unid_hmp, unid_r2h_r2v,
    sql_incra_11, sql_incra, endereco_raw, endereco_unico, endereco, distrito, subprefeitura, 
    zona_de_uso_registro, coord_x, coord_y, georref_nivel, georref_modo, starts_with("ind_"))
...
alvaras_trusted %>%
  count(is.na(sql_incra_11) & descricao_tipo != "Outro" & ind_edificacao_nova == TRUE)

# 7. Exporta -------------------------------------------------------------------

# Assegura a existência do diretório para exportação
fs::dir_create(here("2025_AtualizacaoDadosAlvaras", "3.DadosTratados"))

# Exporta alvarás individualizados
arrow::write_parquet(alvaras_trusted, here("2025_AtualizacaoDadosAlvaras", "3.DadosTratados", 
                                           "alvaras_pde.parquet"))

...
#
alvaras <- arrow::read_parquet(here("2025_AtualizacaoDadosAlvaras", "3.DadosTratados",
                                    "alvaras_pde.parquet")) 

# Para checagem da geocodificação
sf_geo_sp <- st_read(here("2025_AtualizacaoDadosAlvaras", "1.DadosBrutos", "Subprefeituras",
...
alvaras_2 <- tidygeocoder::geocode(
  alvaras_2 %>%
    mutate(
      endereco_key = if_else(!is.na(distrito),
...
alvaras_2 <- alvaras_2 %>%
  ...
  subset(select = -c(endereco_key, coord_x, coord_y))
...
#
sfarrow::st_write_parquet(geo_alvaras, 
                          here("2025_AtualizacaoDadosAlvaras", "3.DadosTratados", 
